...
def NomialAnalysis(query):
    ...
    # Carboxylic Acids / Mineral Acids
        ...
        if query not in descriptions["Acids"] and query not in exceptions["Acids"]:
            ...
            #     return (f"The Chemical Formula for {query} is HCN")
            # elif query.endswith("ic Acid") and query.startswith("Hydro"):
            #     return (f"The Chemical Formula for {query} is " + Classes.Acid(query[5:-7].capitalize() + "ide").formula)
            # elif query.endswith("ic Acid") and not query[:-8] != "o":
            #     return (f"The Chemical Formula for {query} is " + Classes.Acid(query[:-7].capitalize() + "ate").formula)
            # elif query.endswith("ous Acid"):
            #     return (f"The Chemical Formula for {query} is " + Classes.Acid(query[:-8].capitalize() + "ite").formula)
    ...
    if query.endswith("Ether"):
        ...
        if query not in descriptions["Ethers"]:
            ...
            formula (f"The Chemical Formula for {query} is ", end="")
