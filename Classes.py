...
with open("Anions.json", 'r') as file:
    ...
    def __init__(self, anion):
        ...
        for ani, val in anions.items():
            if anion == val[0]:
                ...
                if "sulf" in name.lower(): # Exception
                ...
                if self.name == "Hydrocyanic Acid":
                    self.formula = "HCN"
                if self.name == "Hydrooxic Acid":
                    self.name = "Water"
                if self.name.startswith("Hydro") and self.name.endswith("ic Acid"):
                    self.name = "Hydrogen" + " " + name
                    if self.name == "Hydrogen Sulfuride":
                        self.name = "Hydrogen Sulfide"
                break
...
class OrganicCompound:
    ...
    class Ester:
        def __init__(self, alkylcarbons, carbon):
            ...
            self.formula = (f"C{carbons + 1}H{2 * (carbons + 1)}O2 "
                            f"(or) {"C" + str(carbon) if carbon > 0 else "C" if carbon != 0 else ""}"
                            f"H{(2 * carbon) + 1 if carbon != 0 else ""}COOC{alkylcarbons if alkylcarbons != 1 else ""}H{(2 * alkylcarbons) + 1}")
...
if __name__ == "__main__":
    ...
    for form, values in anions.items():
        if "Bi" not in values[0] and values[0] not in ["Hydroxide", "Peroxide", "Amide", "Hydride", "Arsenide"]:
            name = values[0]
